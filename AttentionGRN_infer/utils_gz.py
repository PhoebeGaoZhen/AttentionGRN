...
def get_pos_neg_T_2(gene_pair_tf_train,gene_pair_target_train, y_train, exp_file, gene_list_file):
    ...
    for i in range(train_label.shape[0]):
        if train_label[i] == 1:
            ...
            print('there other links')

    # positive sample graph
    train_edge_1 = []
    ...
    all_node_feature = pd.read_csv(exp_file, index_col=0).T
    ...
    if len(train_set_node) != all_node_feature.shape[0]:
        ...
        no_feature = pd.DataFrame(0, index=no_feature_x, columns=range(all_node_feature.shape[1]))
...
def get_corr(exp_file, gene_list_file, method, cutoffbeishu):
    ...
    if method == 'cosine':
        ...
        print('CORREALATION ERROR')

    print('The correlation score is calculated')

    # The correlation score matrix was sorted to obtain the score and index from high to low correlation for each gene
    corr_node_list = []
    corr_score_list = []
    ...
    # Set a threshold Select a neighbor with a similar structure.
    # The threshold is the mean + standard deviation # Confirm the virtual edge, which is used to connect nodes with similar structure
...
def buildPE_Kindices(networkx_g, k_hop):
    ...
    # {61: [61], 671: [61, 671], 597: [61, 597], 721: [61, 721], 52: [61, 52], 431: [61, 431]}
...
def getI_directed(nx_g, nx_g_T, IA, k_hop):
    ...
    for target_node in nx_g.nodes():
        ...
        # The source and target k-hop neighbors of this node are found
        neighbors = find123Nei(nx_g, target_node)
        one_hopt = neighbors[0]  #
        two_hopt = neighbors[1]
        three_hopt = neighbors[2]

        # Calculate the minimum maximum mean standard deviation degree of the K-hop neighbor
        T1t = getSI_D_2(nx_g, IA, one_hopt)
        T2t = getSI_D_2(nx_g, IA, two_hopt)

        # print('target_node T1: ', target_node, T1)
        ...
        # print('target_node: ', target_node, T1t, T2t, T1s, T2s)
        if k_hop == 1:
            node_SI.extend(T1s)
            node_SI.extend(T1t)
        elif k_hop == 2:
...
def metric_scores(y_test, y_pred, th=0.5):
    ...
    return AUC, AUPR, Acc, F1, SPE, MCC, Precision, Recall
