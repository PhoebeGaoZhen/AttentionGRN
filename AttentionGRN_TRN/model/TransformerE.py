...
# from dgl.nn import DegreeEncoder, SpatialEncoder, PathEncoder
...
class GT(nn.Module):
    ...
    def __init__(self, in_dim, out_dim, hidden_dim, num_heads, I_dim):
        super().__init__()
        self.h = None
    ...
    def extract_features(self, g):
        ...
        X = g.ndata['x'].to(torch.float32)  # (779,758)
        I = g.ndata['I'].to(torch.float32)  # (779,5)
...
class EXP_Transformer(nn.Module):
    ...
    def forward(self, exp_data):
        ...
        out = self.positionalEncoding(out).to(torch.float32)
...
class AttentionGRN(nn.Module):
    def __init__(self, in_dim_GT, out_dim_GT, hidden_dim, num_heads, I_dim, in_dim_exp, d_model=200, num_classes=2):
        super().__init__()
        self.topo_net = GT(in_dim_GT, out_dim_GT, hidden_dim, num_heads, I_dim)
    ...
    def forward(self, g, exp_data, all_tf, all_target,device, flag_DM):
        if flag_DM:
            ...
            B = self.exp_net(exp_data).to(torch.float32)  # B 32*200


            AA = np.zeros((B.shape[0], 128))
            AA = torch.from_numpy(AA).to(device).to(torch.float32)

            C = torch.cat((AA, B), dim=1, out=None).to(torch.float32)
            out = self.pred_layer(C)
            # out = self.pred_layer(AA)
